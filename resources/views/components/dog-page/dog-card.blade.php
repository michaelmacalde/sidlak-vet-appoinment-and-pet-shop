@props(['dog'])
<div class="group sm:flex rounded-xl" wire:key="{{ $dog->secure_key }}">
    <div class="flex-shrink-0 relative rounded-xl overflow-hidden h-[200px] sm:w-[250px] sm:h-[350px] w-full">
        <img class="absolute top-0 object-cover size-full start-0" src="{{ asset(Storage::url($dog->dog_image[0]['dog_image'])) }}" alt="{{ $dog->dog_name }}">
    </div>
    ...
    <div class="grow">
        ...
        <a href="{{ route('page.dog.single', $dog->dog_slug) }}">
            ...
            <h3 class="text-lg font-semibold text-gray-800 sm:text-2xl group-hover:text-amber-500 dark:text-neutral-300 dark:group-hover:text-amber-500">
                {{ $dog->dog_name }}
            </h3>
            ...
            <div class="mt-5 sm:mt-auto">
                <button wire:click.prevent="adopt({{ $dog->id }})" class="flex flex-row items-center px-6 py-4 text-sm font-medium text-black transition border border-transparent gap-x-2 rounded-xl bg-amber-400 hover:bg-amber-500 disabled:opacity-50 disabled:pointer-events-none focus:outline-none focus:bg-amber-500">
                    ...
                    {{ __('Adopt Me') }}
                    <div wire:loading wire:target="adopt({{ $dog->id }})">
