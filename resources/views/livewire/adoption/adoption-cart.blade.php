...
<div>
    <div class="max-w-[85rem] px-4 sm:px-6 lg:px-8 mx-auto my-10 lg:my-24">
                        ...
                        <div class="p-1.5 min-w-full inline-block align-middle">
                            <div class="overflow-hidden">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                                    <thead>
                                    <tr>
                                        <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase text-start dark:text-neutral-500">{{ __('Image') }}</th>
                                        <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase text-start dark:text-neutral-500">{{ __('Name') }}</th>
                                        <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase text-start dark:text-neutral-500">{{ __('Age') }}</th>
                                        <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase text-start dark:text-neutral-500">{{ __('Gender') }}</th>
                                        <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase text-start dark:text-neutral-500">{{ __('Action') }}</th>
                                    </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                                        @forelse ($this->getAdoptItems as $key => $adoptionItem)
                                        <tr wire:key="{{ $adoptionItem->dog->secure_key }}">
                                            <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-neutral-200">
                                                <div class="flex-shrink-0 relative overflow-hidden h-[70px] sm:w-[70px] sm:h-[70px] w-full rounded-full">
                                                    <img class="absolute top-0 object-cover size-full start-0" src="{{ asset(Storage::url($adoptionItem->dog->dog_image[0]['dog_image'])) }}" alt="{{ $adoptionItem->dog->dog_name }}">
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap dark:text-neutral-200">
                                                <div class="flex flex-col">
                                                   <a href="{{ route('page.dog.single', $adoptionItem->dog->dog_slug) }}">
                                                    <h5 class="font-bold">{{ Str::ucfirst($string = $adoptionItem->dog->dog_name) }}</h5>

                                                    <span class="text-xs italic text-gray-500">{{ $adoptionItem->dog->breed->breed_name }}</span>
                                                    </a>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-neutral-200">{{ $adoptionItem->dog->dog_age }}</td>
                                            <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-neutral-200">{{ $adoptionItem->dog->dog_gender }}</td>
                                            <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-neutral-200">
                                                <button wire:click.prevent='removeAdoptionItem({{ $adoptionItem->id }})' type="submit" class="text-red-500 hover:text-red-700">
                                        ...
                                        <tr>
                                            <td colspan="4">
                                                <div class="w-full mt-4 mb-6 bg-white rounded-xl dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-neutral-700/70">
                                                    <div class="p-4 md:p-10">
                                                        <h5 class="flex flex-row items-center text-gray-800 text-md dark:text-neutral-200">
                                                            <svg class="flex-shrink-0 mr-4 text-amber-500" width="24" height="24" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" />
                                                              </svg>

                                                            {{ __('No items in the selected adoption table') }}
                                                        </h5>
                                                    </div>
        ...
        @if ($this->getAdoptItems->count() > 0)
        <div class="flex flex-row items-end">
            <a href="{{ route('page.app-form') }}" class="flex flex-row items-center px-6 py-4 ml-auto text-sm font-medium text-black transition border border-transparent gap-x-2 rounded-xl bg-amber-400 hover:bg-amber-500 disabled:opacity-50 disabled:pointer-events-none focus:outline-none focus:bg-amber-500">{{ __('Adopt now') }}</a>
        </div>
        @else
        <div class="flex flex-row items-center justify-between">
            ...
            <a class="inline-flex items-center gap-x-1.5 text-sm text-gray-600 decoration-2 hover:underline dark:text-amber-400" href="{{ Auth::user()->hasAnyRole(['admin', 'volunteer', 'super_admin']) ? '/admin' : route('dashboard') }}">
